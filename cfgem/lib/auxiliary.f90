...
! GAC for different exchange coefficients (if available)
!
! NOTE: THE AUXILARY BASIS NEEDS TO BE EXACTLY THE SAME AS COULOMB
!
   double precision, save, allocatable :: aux_mpole_coeff_exch(:)
   double precision, save, allocatable :: aux_sumCH_coeff_exch(:)
   double precision, save, allocatable :: aux_Chermite_coeff_exch(:)
   double precision, save, allocatable :: aux_Dhermite_coeff_exch(:)
   ...
   public FH_AUX_read, &
          ...
          herm_coeff_off_for_aux_Dprim, &
          aux_Dhermite_coeff, &
          aux_Dhermite_coeff_exch, & !GAC
          sumCH_order_for_site_type, &
          sumCH_level_for_site_type, &
          sumCH_coeff_off_for_site_type, &
          aux_sumCH_coeff, &
          aux_sumCH_coeff_exch, & !GAC
          mpole_order_for_site_type, &
          mpole_level_for_site_type, &
          mpole_coeff_off_for_site_type, &
          aux_mpole_coeff, &
          aux_mpole_coeff_exch !GAC
...
! Local variables:
   ...
   aux_mpole_coeff=0.d0
   ! GAC for different exch coefs
   allocate(aux_mpole_coeff_exch(tot_num_aux_mpole_coeff),stat=ier1) 
   if ( ier1 /= 0 )then
      write(out_lun,*)'FH_AUX_read: allocate fails for aux_mpole_coeff_exch!'
      stop
   endif
   aux_mpole_coeff_exch=0.d0
   ! GAC for different exch coefs
   ...
   if ( tot_num_aux_Cprim > 0 )then
      ...
      herm_exp_for_aux_Cprim=0.d0
      herm_rec_exp_for_aux_Cprim=0.d0
      herm_ord_for_aux_Cprim=0.d0
      herm_lev_for_aux_Cprim=0.d0
      herm_coeff_off_for_aux_Cprim=0.d0
   ...
   if ( tot_num_aux_Dprim > 0 )then
      ...
      herm_exp_for_aux_Dprim=0.d0
      herm_rec_exp_for_aux_Dprim=0.d0
      herm_ord_for_aux_Dprim=0.d0
      herm_lev_for_aux_Dprim=0.d0
      herm_coeff_off_for_aux_Dprim=0.d0
   ...
   if ( CD_split_expon > small_value ) then
      ...
      aux_sumCH_coeff=0.d0
      !GAC
      allocate(aux_sumCH_coeff_exch(tot_num_aux_sumCH_coeff),stat=ier1)
      if ( ier1 /= 0 )then
         write(out_lun,*)'FH_AUX_read: allocate fails!'
         stop
      endif
      aux_sumCH_coeff_exch=0.d0
   else
   ...
   if ( tot_num_aux_Cprim > 0 )then
      ...
      aux_Chermite_coeff=0.d0
   endif !( tot_num_aux_Cprim > 0 )then
   if ( tot_num_aux_Cprim > 0 )then
      allocate(aux_Chermite_coeff_exch(tot_num_aux_Cherm_coeff),stat=ier1)
      if ( ier1 /= 0 )then
         write(out_lun,*)'FH_AUX_read: bad allocate of herm exch coeff array'
         stop
      endif
      aux_Chermite_coeff_exch=0.d0
   endif !( tot_num_aux_Cprim > 0 )then
   if ( tot_num_aux_Dprim > 0 )then
   ...
   if ( tot_num_aux_Dprim > 0 )then
      ...
      aux_Dhermite_coeff=0.d0
   endif !( tot_num_aux_Dprim > 0 )then
   ! GAC
   if ( tot_num_aux_Dprim > 0 )then
      allocate(aux_Dhermite_coeff_exch(tot_num_aux_Dherm_coeff),stat=ier1)
      if ( ier1 /= 0 )then
         write(out_lun,*)'FH_AUX_read: bad allocate of exch herm coeff array'
         stop
      endif
      aux_Dhermite_coeff_exch=0.d0
   endif !( tot_num_aux_Dprim > 0 )then
   ...
   do n = 1,num_aux_site_types
      ...
      if ( mp_order > 0 )then
         ...
         ! GAC for exch coefs
         aux_mpole_coeff_exch(offc+1) = mp_coeff
         ! GAC for exch coefs
      endif
      do j = 2,mp_order
         read(free_lun,*,iostat=ios)mp_coeff
         ...
         ! GAC for exch coefs
         aux_mpole_coeff_exch(offc+j) = mp_coeff
         ! GAC for exch coefs
      enddo
      ncprim = 0
      ndprim = 0
...
! GAC
               aux_Chermite_coeff_exch(herm_off+1) = coeff
! GAC
               do j = 2,order
                  read(free_lun,*,iostat=ios)coeff
                  if ( ios /= 0 )then
...
! GAC
                  aux_Chermite_coeff_exch(herm_off+j) = coeff
! GAC
               enddo
               if ( CD_split_expon > small_value ) then
                  do j = 1,order
...
! GAC
               aux_Dhermite_coeff_exch(herm_off+1) = coeff
! GAC
               do j = 2,order
                  read(free_lun,*,iostat=ios)coeff
                  if ( ios /= 0 )then
...
! GAC
                  aux_Dhermite_coeff_exch(herm_off+j) = coeff
! GAC
               enddo
            endif
         enddo !k = 1,nprim
         ...
         ! GAC
               aux_sumCH_coeff_exch(herm_off+j) = sum_coeff(j)
         ! GAC
            enddo
         endif
      endif !nprim > 0
...
subroutine FH_AUX_deallocate()
   ...
   !GAC
   if ( allocated(aux_mpole_coeff_exch) ) deallocate(aux_mpole_coeff_exch)
   if ( allocated(aux_sumCH_coeff_exch) ) deallocate(aux_sumCH_coeff_exch)
   ...
   !GAC
   if ( allocated(aux_Chermite_coeff_exch) ) &
               deallocate(aux_Chermite_coeff_exch)
   ...
   !GAC
   if ( allocated(aux_Dhermite_coeff_exch) ) &
               deallocate(aux_Dhermite_coeff_exch)
