...
subroutine FH_hermite_load(out_lun)
   ...
   use user,only : gaussian_extent_tol,HC_grid_number_for_Caux, &
                   ...
                   do_overlap ! GAC for exchange coeffs if avail
...
! Local variables:
   ...
   do n = 1,num_sites
      ...
      if ( order > 0 )then
         if (do_overlap .ne. 1) then ! GAC: Using Coulomb Coefs
            do k = 1,order
               Local_multipole_coeff(offcoeff+k) = &
                    aux_mpole_coeff(offauxcoeff+k)
            enddo
         else ! GAC: Using Exchange Coeffs
            do k = 1,order
               Local_multipole_coeff(offcoeff+k) = &
                    aux_mpole_coeff_exch(offauxcoeff+k)
            enddo
         endif
      endif
   enddo
      ...
      do n = 1,num_sites
         ...
         if ( order > 0 )then
            if (do_overlap .ne. 1) then ! GAC: Using Coulomb Coefs
               ...
               enddo
            else ! GAC: Using Exch Coefs
               do k = 1,order
                  Local_sumCH_coeff(offcoeff+k) =  &
                       aux_sumCH_coeff_exch(offauxcoeff+k)
               enddo
            endif
         endif
      enddo
      ...
      do n = 1,num_sites
         ...
         do k = 1,numprim
            ...
            if (do_overlap .ne. 1) then ! GAC: Using Coulomb Coefs
               ...
               do j = 1,order
                  Local_Chermite_coeff(offcoeff+j) =  &
                            aux_Chermite_coeff_exch(offauxcoeff+j)
               enddo
            endif
             !print *,'cherm ',Local_Chermite_coeff(offcoeff+j)
         enddo
   ...
   if ( tot_num_herm_Dprims > 0 )then
      ...
      do n = 1,num_sites
         ...
         do k = 1,numprim
            ...
            if (do_overlap .ne. 1) then ! GAC: Using Couolmb Coefs
               do j = 1,order
             ...
             !print *,'dherm ',Local_Dhermite_coeff(offcoeff+j)
         enddo
         if ( max_order > max_xform_order_for_site(n) ) &
               max_xform_order_for_site(n) = max_order
      enddo 
   ...
   if ( verbose == 1 )then
    ...
    !do n = 1, tot_num_Cherm_coeffs
    !   print *,'dherm ',Local_Dhermite_coeff(n)
    !enddo
...
subroutine FH_hermite_local_to_global()
   ...
   do n = 1,num_sites
      ...
      !if (order == 1)then
      !   print *,n,Global_multipole_coeff(coeff_off+1),&
      !             Local_multipole_coeff(coeff_off+1)
      ...
      !print *,'site,global,local'
      !if (order == 1)then
      !   print *,n,Global_Chermite_coeff(coeff_off+1),&
      !             Local_Chermite_coeff(coeff_off+1)
      !else if (order == 4)then
      !   do j=1,3
      !      print *,n,Global_Chermite_coeff(coeff_off+j),&
      !                Local_Chermite_coeff(coeff_off+j)
      !   enddo
      !else if (order == 10)then
      !   do j=1,10
      !      print *,n,Global_Chermite_coeff(coeff_off+j),&
      ...
      !print *,'Diffuse HERMITES'
      !print *,'site,global,local'
      !if (order == 1)then
      !   print *,n,Global_Dhermite_coeff(coeff_off+1),&
      !             Local_Dhermite_coeff(coeff_off+1)
      !else if (order == 4)then
      !   do j=1,3
      !      print *,n,Global_Dhermite_coeff(coeff_off+j),&
      !                Local_Dhermite_coeff(coeff_off+j)
      !   enddo
      !else if (order == 10)then
      !   do j=1,10
      !      print *,n,Global_Dhermite_coeff(coeff_off+j),&
