...
c      character*4 pa,pb,pc,pd,pe
c      character*20 blank,pt
c      character*20 pt1,pt2
...
c      ntt = maxntt
c      do i = maxntt, 1, -1
c         if (ktt(i) .eq. blank)  ntt = i - 1
c      end do
      if (allocated(tortorglob)) deallocate(tortorglob)
      allocate (tortorglob(nbitorloc))
      ntortorloc = 0
      do i = 1, nbitorloc
...
c
         do j = 1, ntt
            if (ktt_sys(j) .eq. pt1) then
               ntortorloc = ntortorloc + 1
               ntortorloc1 = ntortorloc1 + 1
               tortorglob(ntortorloc) = tortorcount + ntortorloc1
               goto 90
            else if (ktt_sys(j) .eq. pt2) then
               ntortorloc = ntortorloc + 1
               ntortorloc1 = ntortorloc1 + 1
               tortorglob(ntortorloc) = tortorcount + ntortorloc1
               goto 90
