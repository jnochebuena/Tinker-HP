...
c                 pt = pc//pb//pa
              ...
              do j = 1, maxnsb
c                 if (ksb(j).eq.blank .or. ksb(j).eq.pt) then
...
c              call numeral (itc,pc,size)
              if (ita .le. itc) then
                 call front_convert_base3(ita,itb,itc,ipt)
c                 pt = pa//pb//pc
              else
                 call front_convert_base3(itc,itb,ita,ipt)
c                 pt = pc//pb//pa
              end if
              do j = 1, nsb
...
c        end if
        ...
        do j = 1, nsb
           if (ksb_sys(j) .eq. ipt) then
              ...
              strbndglob(nstrbndloc) = strbndcount + nstrbndloc1
              goto 70
           end if
        end do
 70     continue
