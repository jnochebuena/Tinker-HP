...
c
      subroutine reassignrespa(fast,ialt,naltloc)
      use atoms
      use bound
      use cell
      use domdec
...
c
        do iproc = 1, nrecdir_recep
          if (precdir_recep(iproc).ne.rank) then
            do i = 1, domlen(precdir_recep(iproc)+1)
              ilocrec = bufbegrec(precdir_recep(iproc)+1)+i-1
              buffers(1,ilocrec) = dedsprec(1,ilocrec)
              buffers(2,ilocrec) = dedsprec(2,ilocrec)
              buffers(3,ilocrec) = dedsprec(3,ilocrec)
            end do
          end if
        end do
c
        ...
        do i = 1, nrecdir_send
          if (precdir_send(i).ne.rank) then
            ...
            do j = 1, nloc
              iglob = glob(j)
              ilocrec = locrec1(iglob)
              dedsp(1,j) = dedsp(1,j) + dedsprec(1,ilocrec)
              dedsp(2,j) = dedsp(2,j) + dedsprec(2,ilocrec)
              dedsp(3,j) = dedsp(3,j) + dedsprec(3,ilocrec)
            end do
          end if
        end do
      end  if
