...
c     GAC, refresh arrays going to GEM LIB
...
c
     ...
     $           aalt(j,iglob))*dt_x
            ...
            !GAC to put atom positions in GEM arrays
            if (useGEM) then
               if (rank .eq. 0) then
      ...
      !print *,'after copying tinker crds'
      !do h = 1, n
      !   print *,atom_crds(1,h),atom_crds(2,h),atom_crds(3,h)
         !print *,v(1,h),v(2,h),v(3,h)
         !print *,x(h),y(h),z(h)
      !enddo
      !print *,'------------------'
c
