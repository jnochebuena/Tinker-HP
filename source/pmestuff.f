...
c
     ...
     $               k-kstart3+1,iproc+1) + term0*t0 + term1*t1 +
     $               term2*t2
                   goto 10
                 end if
               end do
...
c
     ...
     $             ((j.ge.jstart3).and.(j.le.jend3)).and.
     $             ((i.ge.istart3).and.(i.le.iend3))) then
                   qgrid2loc(1,i-istart3+1,j-jstart3+1,k-kstart3+1,
     $               iproc+1)
     $              = qgrid2loc(1,i-istart3+1,j-jstart3+1,k-kstart3+1,
     $              iproc+1) + term01*t0 + term11*t1
                   qgrid2loc(2,i-istart3+1,j-jstart3+1,k-kstart3+1,
     $               iproc+1) = qgrid2loc(2,i-istart3+1,j-jstart3+1,
     $               k-kstart3+1,iproc+1) + term02*t0
     &                            + term12*t1
                   goto 10
                 end if
               end do
...
c
     ...
     $             ((j.ge.jstart3).and.(j.le.jend3)).and.
     $             ((i.ge.istart3).and.(i.le.iend3))) then
                   tq  = qgridin_2d(1,i-istart3+1,j-jstart3+1,k-
     $              kstart3+1,iproc+1)
                   goto 10
                 end if
               end do
...
c
     ...
     $             ((j.ge.jstart3).and.(j.le.jend3)).and.
     $             ((i.ge.istart3).and.(i.le.iend3))) then
                   tq_1  = qgrid2in_2d(1,i-istart3+1,j-jstart3+1,
     $               k-kstart3+1,iproc+1)
                   tq_2  = qgrid2in_2d(2,i-istart3+1,j-jstart3+1,
     $               k-kstart3+1,iproc+1)
                   goto 10
                 end if
               end do
...
c
c     "fphi_uind_site_solv" extracts the induced dipole potential at the i-th site from
c     the particle mesh Ewald grid during iterative solver
c
c
      subroutine fphi_uind_site_solv(isite,impi,fdip_phi1,fdip_phi2)
      use domdec
      use fft
      use pme
      use potent
...
c
cc
cc     find the matrix to convert Cartesian to fractional
cc
...
c
c
      subroutine cmp_to_fmp_site(ctf,cmp,fmp)
      use sizes
      implicit none
      integer j,k
      real*8 ctf(10,10)
