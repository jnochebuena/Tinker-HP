...
c
     ...
     &                    - 2.0d0*ci*ck*dspscale(kglob)*damp*ddamp/r7
c
c     increment the overall damped dispersion energy component
c
                  edsp = edsp + e
...
c
 ...
 100               continue
         ...
         dedsprec(1,iloc) = dedsprec(1,iloc) + fi*(recip(1,1)*de1
     &                            +recip(1,2)*de2+recip(1,3)*de3)
         dedsprec(2,iloc) = dedsprec(2,iloc) + fi*(recip(2,1)*de1
     &                            +recip(2,2)*de2+recip(2,3)*de3)
         dedsprec(3,iloc) = dedsprec(3,iloc) + fi*(recip(3,1)*de1
