...
c
      ...
      real*8 vdw1,vdw2,tap1,tap2,tap3,tap4,tap5
...
c            call numeral (id,pd,size)
            ndi = ndi + 1
            call front_convert_base(ia,ib,ic,id,0,kdi(ndi))
...
c            call numeral (id,pd,size)
            nti = nti + 1
            call front_convert_base(ia,ib,ic,id,0,kti(nti))
...
c
            ...
            pal = 0.0d0
            string = record(next:120)
            read (string,*,err=780,end=780)  ia,pel,pal
  780       continue
            if (ia .ne. 0) then
               cpele(ia) = abs(pel)
               cpalp(ia) = pal
...
c
            ...
            adp = 0.0d0
            string = record(next:120)
            read (string,*,err=680,end=680)  ia,cdp,adp
  680       continue
            if (ia .ne. 0) then
               dspsix(ia) = cdp
               dspdmp(ia) = adp
