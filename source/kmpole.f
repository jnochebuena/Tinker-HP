...
c      if (associated(zaxis)) then
c        CALL MPI_Win_shared_query(winzaxis, 0, windowsize, disp_unit,
c     $  baseptr, ierr)
c        CALL MPI_Win_free(winzaxis,ierr)
c      end if
c      if (associated(yaxis)) then
c        CALL MPI_Win_shared_query(winyaxis, 0, windowsize, disp_unit,
c     $  baseptr, ierr)
c        CALL MPI_Win_free(winyaxis,ierr)
c      end if
c      if (associated(zaxis)) then
c        CALL MPI_Win_shared_query(winxaxis, 0, windowsize, disp_unit,
c     $  baseptr, ierr)
c        CALL MPI_Win_free(winxaxis,ierr)
c      end if
...
c        windowsize = int(n,MPI_ADDRESS_KIND)*4_MPI_ADDRESS_KIND
c      else
...
c     $  hostcomm, baseptr, winzaxis, ierr)
c      if (hostrank /= 0) then
c        CALL MPI_Win_shared_query(winzaxis, 0, windowsize, disp_unit,
c     $  baseptr, ierr)
c      end if
...
cc
c      CALL C_F_POINTER(baseptr,zaxis,arrayshape)
cc
cc     yaxis
cc
c      arrayshape=(/n/)
...
cc
c      CALL MPI_Win_allocate_shared(windowsize, disp_unit, MPI_INFO_NULL,
c     $  hostcomm, baseptr, winyaxis, ierr)
c      if (hostrank /= 0) then
c        CALL MPI_Win_shared_query(winyaxis, 0, windowsize, disp_unit,
c     $  baseptr, ierr)
c      end if
...
cc    association with fortran pointer
cc
c      CALL C_F_POINTER(baseptr,yaxis,arrayshape)
cc
cc     xaxis
cc
c      arrayshape=(/n/)
...
cc
c      CALL MPI_Win_allocate_shared(windowsize, disp_unit, MPI_INFO_NULL,
c     $  hostcomm, baseptr, winxaxis, ierr)
c      if (hostrank /= 0) then
c        CALL MPI_Win_shared_query(winxaxis, 0, windowsize, disp_unit,
c     $  baseptr, ierr)
c      end if
...
cc    association with fortran pointer
cc
c      CALL C_F_POINTER(baseptr,xaxis,arrayshape)
c
c     pole
c
      arrayshape2=(/13,n/)
