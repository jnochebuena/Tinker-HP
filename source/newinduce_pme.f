...
c
      ...
      real*8 hsq,expterm
      real*8 term,pterm
      real*8 cmp(10),fmp(10),cphi(10,*),ctf(10,10)
...
c
        call cmp_to_fmp_site(ctf,cmp,fmp)
c
c     assign PME grid
c
...
cc
c      do i = 1, nrec_recep
c        call aadd(2*n1mpimax*n2mpimax*n3mpimax,
...
c      do i = 1, nrec_recep
c        call aadd(2*n1mpimax*n2mpimax*n3mpimax,
c     $   qgrid2in_2d(1,1,1,1,1),qgridmpi(1,1,1,1,i),
c     $   qgrid2in_2d(1,1,1,1,1))
c      end do
c
c     Perform 3-D FFT forward transform
c
...
c
      do i = 1, npolerecloc
        ...
        iglob = ipole(iipole)
        iloc  = poleloc(iipole)
        call fphi_uind_site_solv(iglob,i,fdip_phi1,fdip_phi2)
c        call fphi_uind_site(iglob,i,fdip_phi1,fdip_phi2,fdip_sum_phi)
        if (.not.(use_pmecore).and.(repart(iglob).eq.rankloc)) then
c
