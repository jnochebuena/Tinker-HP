...
c     ################################################################
c     ##                                                            ##
c     ##  subroutine ehal1  --  mod buffered 7 energy & derivatives  ##
c     ##                                                            ##
c     ################################################################
c
c
c     "ehal1" calculates the buffered 7 modified van der Waals energy and
c     its first derivatives with respect to Cartesian coordinates
c     FOR GEM* because we have explicit exchange
c
c
      subroutine ehalgem
      use energi
...
c     ################################################################
c     ##                                                            ##
c     ##  subroutine ehal1c_gem  --  buffered 7 vdw derivs via list  ##
c     ##                                                            ##
c     ################################################################
c
c
c     "ehal1c_gem" calculates the buffered 14-7 van der Waals energy and
c     its first derivatives with respect to Cartesian coordinates
c     using a pairwise neighbor list
c
c
      subroutine ehal1c_gem
      use atmlst
      use atoms
      use bound
      use couple
...
c     S:JORGE
            ...
            !if (proceed) then
               !proceed=((usei .or. use(kglob)) .or. (use(kv) .or. QMMM))
            !end if 
c
...
! GAC: ONLY USE r^-7 term because GEM has explicit EXCHANGE
c
