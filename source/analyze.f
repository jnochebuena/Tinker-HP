...
c
     ...
     &          11x,''Kcal/mole'',6x,''Interactions''/)'
      write (iout,fstr)
      if (neb.ne.0 .and. .not. onlyXC) then
         fstr = '('' Bond Stretching'',12x,'//form1//')'
         write (iout,fstr)  eb,neb
      end if
      if (nea.ne.0 .and. .not. onlyXC) then
      ...
      if (neub.ne.0 .and. .not. onlyXC) then
...
c     S:JORGE
      if (.not. onlyXC .and. usargem .or. onlyHal) then
         fstr = '('' Halgren Energy'',13x,'//form1//')'
         write (iout,fstr)  ene_hal
      end if
      if (.not. onlyHal .and. usargem .or. onlyXC) then
         fstr = '('' Exchange Energy'',12x,'//form1//')'
         write (iout,fstr)  ene_gem_exch
      end if
c     E:JORGE
...
c     S:JORGE
      if (.not. onlyHal .and. usargem .or. onlyXC) then
         fstr = '('' Coulomb Energy'',13x,'//form1//')'
         write (iout,fstr)  ene_gem_coul
      end if
c     E:JORGE
      if (nep.ne.0) then
