...
c         else if (barostat .eq. 'LANGEVIN') then
c            call plangevin(dt,pres,istep)
         else if (barostat .eq. 'MONTECARLO') then
            !!GAC for GEM
            !if (useGEM) then
            !  if (allocated(atom_crds)) deallocate(atom_crds)
            !  allocate(atom_crds(3,atom_cnt),stat=alloc_failed)
            !  if (alloc_failed .ne. 0) then
            !    print *,'Allocation of arrays failed!'
            !    call fatal
            !  end if
            !  atom_crds(1,:) = x(:)
            !  atom_crds(2,:) = y(:)
...
c
      subroutine plangevin (dt,pres,istep)
      use atoms
      use bath
      use boxes
      use domdec
      ...
      use mpi
      implicit none
      integer i,j,k,iglob,istep,ierr
      integer start,stop
      real*8 dt,pres
      real*8 scale,third
      real*8 a1,a2,R,normal,maxwell
      third = 1.0d0 / 3.0d0
c      a1 = exp(-gammapiston*dt)
...
cc
cc
c        vextvol = vextvol + 0.5*dt*aextvol
c        extvol = extvol + 0.5*dt*vextvol
c        R = normal()
c        vextvol = a1*vextvol + a2*R/sqrt(masspiston)
c        extvol = extvol + 0.5*dt*vextvol
cc
c        aextvol = convert*(pres-atmsph)/(prescon*masspiston)
cc
c        vextvol = vextvol + 0.5*dt*aextvol
cc
c      end if
c      write(*,*) 'xbox avant =',xbox
c      call MPI_BCAST(extvol,1,MPI_REAL8,0,COMM_TINKER,ierr)
c      call MPI_BCAST(vextvol,1,MPI_REAL8,0,COMM_TINKER,ierr)
c      scale  =  (extvol/extvolold)**third
c
c      xbox = (extvol)**third!xbox * scale
c      ybox = (extvol)**third!ybox * scale
c      zbox = (extvol)**third!zbox * scale
cc
...
c         iglob = glob(i)
c         if (use(iglob)) then
c            x(iglob) = x(iglob) * scale
c            y(iglob) = y(iglob) * scale
c            z(iglob) = z(iglob) * scale
c         end if
c      end do
c      if (rank.eq.0) then
c      write(*,*) 'volbox apres = ',volbox,'extvol = ',extvol,
c     $   'extvolold = ',extvolold
c      end if
cc
cc   also rescale xbegproc, xendproc...
cc
c            call ddpme3dnpt(scale,istep)
cc
      return
      end
c
...
c
      ...
      integer start,stop
      integer alloc_failed ! GAC
      !integer tot_mcbar_attempts, mcbar_attempts
            ...
            if (volscale .eq. 'MOLECULAR') then
               ...
               do i = 1, nmoleloc
                  ...
                  do j = start, stop
                     ...
                     if (use(k)) then
                        ...
                        y(k) = y(k) + ymove
                        z(k) = z(k) + zmove
                        ! GAC to calculate energy with updated crds
                        if (useGEM) then
                           atom_crds(1,k) = x(k)
                           atom_crds(2,k) = y(k)
                           atom_crds(3,k) = z(k)
              ...
              do i = 1, nbloc
                 ...
                 if (use(iglob)) then
                    ...
                    ! GAC to calculate energy with updated crds
                    if (useGEM) then
                       atom_crds(1,iglob) = x(iglob)
                       atom_crds(2,iglob) = y(iglob)
                       atom_crds(3,iglob) = z(iglob)
                    endif
                    ! GAC to calculate energy with updated crds
...
c
         if (volscale .eq. 'MOLECULAR') then
           ...
           dkin = dble(nmol) * kt * log(volbox/volold)
         else
...
c
         ...
         call MPI_BCAST(valrand,1,MPI_REAL8,0,COMM_TINKER,ierr)
         !if(valrand.le.expterm) print *,'accepting step in mcbar'
         if (valrand .ge. expterm) then
            !print *,'rejecting step in mcbar'
            epot = eold
            xbox = xboxold
            ybox = yboxold
            zbox = zboxold
