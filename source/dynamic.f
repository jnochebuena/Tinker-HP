...
! GAC start
! Let user know GEM will be used and check barostat if pressure control
      if (useGEM) then
         ! failsafe, if useGEM in mdinit should set this to MC
         ! make sure pressure control is ok (no virial yet) if NPT
         ! if not, exit cleanly
            if (barostat .ne. 'MONTECARLO') then
               write (iout,466)
  466       format (/,' GEM can only use MonteCarlo barostat, check',
     &             ' keyfile; exiting')
               call fatal
            endif
         !endif
...
c
      ...
      do istep = 1, nstep
         ...
         !else if (integrate .eq. 'METRO') then
         !  call metro(istep,dt)
         else
