...
c
c     S:JORGE
      if (use_vdw .and. .not. useGEM) then
         if (.not. onlyHal .and. .not. onlyXC) then
...
c     S:JORGE
      if (use_mpole .and. .not. useGEM) then
         if (.not. onlyHal .and. .not. onlyXC) then
            call empole3
         end if
      end if
...
c
     ...
     &          + et + ept + ebt + ett + ev + er + edsp + ect+ em
     &          + ec + ep +  eg + ex
      energy = esum

      if (onlyXC) energy = ene_gem_exch + ene_gem_coul
c
