...
! # of ffts : 5 (efi, r0, Tr0, T2r0, T3r0)
      ...
      integer :: ii, ix, iy, iz, alphac, jj, jx, jy, jz, betac, kk, i,
     $           j, iipole, irhs, k,jxloc,jyloc,jzloc,jjj,jglob,
     $           ierr,iglob,ilocrec,jjpole,jloc,jpoleloc,kglob,kkpole,
     $           kloc
     ...
     $          a13, a14, a20, a21, a22, a23, a31, a41, a32
      ...
      real*8, allocatable, dimension(:,:,:,:,:) :: dtorquetheta,
     $ dtorquethetabis
...
c      write(*,*) 'time solver = ',time1-time0
      ...
      time0 = mpi_wtime()
      call scalderfieldzmat6(nrhs, arr_ded,    arr_dep, 
     $                                 arr_dtr0,   r0(:,1,:),
     $                                 arr_dTTr0,  Tr0(:,1,:), 
     $                                 arr_dTT2r0, T2r0(:,1,:), 
     $                                 ade, adme, adte, adtb)
...
c         write(*,*) 'time scalderreal = ',time1-time0
      ...
      call scalderfieldzmatrec6(nrhs, 
     $      arr_dep,arr_depbis,fphiarrdep,arr_ded,arr_dedbis,fphiarrded,
     $      arr_dtr0,arr_dtr0bis,fphiarrdtr0,r0(:,1,:),r0bis(:,1,:),
     $      fphir0(:,1,:),arr_dttr0,arr_dttr0bis,fphiarrdttr0,
     $      tr0(:,1,:),tr0bis(:,1,:),fphitr0(:,1,:),
     $      arr_dtt2r0,arr_dtt2r0bis,fphiarrdtt2r0,t2r0(:,1,:),
     $      t2r0bis(:,1,:),fphit2r0(:,1,:),adebis, admebis, adtebis,
     $      adtbbis)
...
! Only guess is available (+ prec via 'precond')
      ...
      logical :: doi, doix, doiy, doiz, precond
      integer :: ii, ix, iy, iz, alphac, jj, jx, jy, jz, betac, kk, i,
     $           j, iipole, irhs, k, jxloc,jyloc,jzloc,jjj,jglob,
     $           ierr,iglob,ilocrec,jjpole,nrhs,jloc,jpoleloc,kglob,
     $           kkpole,kloc
      integer :: status(MPI_STATUS_SIZE)
      integer, allocatable :: reqrec(:),reqsend(:)
      integer, allocatable :: reqrecdirrec(:),reqrecdirsend(:)
     ...
     $ buffermpimu2,req2rec,req2send)
      ...
      !Contraction
      call scalderfieldzmatrec8(nrhs, 
     $    arr_ded,arr_dedbis,fphiarrded,arr_dep,arr_depbis,fphiarrdep, 
     $    arr_dtr0,arr_dtr0bis,fphiarrdtr0,r0(:,1,:),r0bis(:,1,:),
     $    fphir0(:,1,:),arr_dttr0,arr_dttr0bis,fphiarrdttr0,
     $    tr0(:,1,:),tr0bis(:,1,:),fphitr0(:,1,:),
     $    arr_dtt2r0,arr_dtt2r0bis,fphiarrdtt2r0,t2r0(:,1,:),
     $    t2r0bis(:,1,:), fphit2r0(:,1,:),-arrA,
     $     -arrAbis, -fphiarrA,mu0(:,1,:),mu0bis(:,1,:),fphimu0(:,1,:), 
     $    adebis, admebis, adtebis,adtbbis)
      ...
      call scalderfieldzmat8(nrhs, arr_ded, arr_dep, 
     $      arr_dtr0,   r0(:,1,:),
     $      arr_dTTr0,  Tr0(:,1,:), 
     $      arr_dTT2r0, T2r0(:,1,:), 
     $      -arrA,      oldaefi(:,1,:),
...
c
      ...
      call scalderfieldzmatrec6(nrhs, 
     $       arr_dep,arr_depbis,fphiarr_dep,arr_ded,arr_dedbis,
     $       fphiarr_ded, 
     $       arr_dtr0,arr_dtr0bis,fphiarrdtr0,
     $       efi(:,1,:),efibis(:,1,:),
     $       fphie(:,1,:),arr_dttr0,arr_dttr0bis,
     $       fphiarrdttr0,
     $       tefi(:,1,:),tefibis(:,1,:), fphite(:,1,:),
     $       arr_dtt2r0,arr_dtt2r0bis,
     $       fphiarrdtt2r0,t2efi(:,1,:),t2efibis(:,1,:),
     $       fphit2e(:,1,:),
     $       adebis, admebis, adtebis, adtbbis)
...
! TCG2 with guess AND peek-step, NO preconditioner
     ...
     $       tr0bis(:,1,:),
     $       fphitr0(:,1,:),arr_dtt2r0,
     $       arr_dtt2r0bis,fphiarrdtt2r0,T2r0(:,1,:),
     $       T2r0bis(:,1,:),
     $       fphit2r0(:,1,:),
     $       -arr_dr0,-arr_dr0bis,-fphiarrdr0,oldaefi(:,1,:),
     $       oldaefibis(:,1,:),
     $       fphiaE(:,1,:),
     $       adebis, admebis, adtebis, adtbbis)
...
! aboard
      ...
      use mpi
      implicit none
      logical :: doi, doix, doiy, doiz, precond, isguess, peek
      integer :: ii, ix, iy, iz, alphac, jj, jx, jy, jz, betac, kk, i,
     $           j, iipole, irhs, k, jxloc,jyloc,jzloc,jjj,jglob,
     $           ierr,iglob,ilocrec,jjpole,nrhs,jloc,jpoleloc,kglob,
     $           kkpole,kloc
     ...
     $            Toefi,Toefibis,Toefirec,Toefirecbis,
     $            T2oefi,T2oefibis,T2oefirec,T2oefirecbis,
     ...
     $            tmpfphi2,tmpbig,tmpbigbis,fphitmp,fphitoefi,
     $            fphioldefi,fphit2oefi,fphioldr0,fphioldtr0,fphioldt2r0
      real*8, allocatable, dimension(:,:) :: torq_t, torq_mu, grad_ene,
...
c      write(*,*) 'a10 = ',a10
     ...
     $       t2r0bis(:,1,:),
     $       fphit2r0(:,1,:),-arr_dr0,
     $       -arr_dr0bis,-fphiarrdr0,oldaefi(:,1,:),
