...
c     ####################################################################################
c     ##                                                                                ##
c     ##  subroutine epolar1tcg1_2  --  Ewald polarization energy and grad with tcg1    ##
c     ##                                                                                ##
c     ####################################################################################
c
c
...
c  
      ...
      integer  :: nrhs
      logical :: doi, doix, doiy, doiz, isguess, peek, precond
      integer :: ii, iz,ix,iy, alphac, jj, jx, jy, jz, kk, betac, irhs,
     $            i, iipole,jjpole, j, tag, ierr,iglob,ilocrec,
     $            kkpole,kglob,kloc
     ...
     $     buffermpimu,req2rec,req2send)
          ...
          call scalderfieldzmat3(nrhs, arr_ded, arr_dep, 
     $                               arr_dtr0, r0(:,1,:),
     $                               -arrA, oldaefi(:,1,:),
     $                               ade, adme, adte, adtb)
     ...
     $     buffermpimu,req2rec,req2send)
          ...
          call scalderfieldzmatrec3(nrhs,xx,arr_dedbis,
     $         fphiarrded,xx,arr_depbis,fphiarrdep,
     $         xx,arr_dtr0bis,fphiarrdtr0,xx,r0bis(:,1,:),
     $         fphir0(:,1,:),xx,
     $         -arrAbis, -fphiarrA,xx,oldaefibis(:,1,:), 
     $         fphimu0(:,1,:),adebis, admebis, adtebis, adtbbis)
        ...
        if (rank.le.ndir-1) then
          call scalderfieldzmat1(nrhs, arr_ded, arr_dep, 
     $     arr_dtr0,r0(:,1,:),ade,adme,adte,adtb)
          denedr =   ade(:,1,:)  + ade(:,2,:)  + adtb    
          denedmu=  adme(:,1,:)   + adme(:,2,:)    
          denedt =  adte(:,:,1,:) + adte(:,:,2,:)
        else
          call scalderfieldzmatrec1(nrhs,xx,arr_dedbis,
     $      fphiarrded,xx,arr_depbis,fphiarrdep,
     $      xx,arr_dtr0bis,fphiarrdtr0,xx,r0bis(:,1,:),
     $      fphir0(:,1,:),adebis, admebis, adtebis, adtbbis)
