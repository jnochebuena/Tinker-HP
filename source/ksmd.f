...
c
         if (use_smd_velconst) then
             if (SMDk .lt. 0.0d0 .and. rank.eq.0) then
                write (ismdout,21)
   21           format(3x,'Problem with SMDk; Negative ',
     &          3x,'Value => has to be a positive real or nul !')
             ...
             if (SMDk == 0.0d0 .and. rank.eq.0) then
                write (ismdout,22) 
   22           format(3x,'Problem with SMDk: equal to 0',
     &  /,3x,'=> Check the key file !')
             ...
             if (SMDVel == 0.0d0 .and. rank.eq.0) then
                write (ismdout,23) 
...
c => We find here a single position restrain !
             ...
             if (SMDVel .lt. 0.0d0 .and. rank.eq.0) then
                write (ismdout,24) 
...
c CFSMD case
         if (use_smd_forconst) then
             if (SMDFor == 0.0d0 .and. rank.eq.0) then
                write (ismdout,25) 
   25           format(3x,'Problem with SMDFor : ',
     & 'equal to 0',/,3x,'=> Check the key file !')
             ...
             if (SMDFor .lt. 0.0d0 .and. rank.eq.0) then
                write (ismdout,20)
...
c CVSMD case
         if (use_smdk2) then
             if (SMDk2 .lt. 0.0d0 .and. rank.eq.0) then
                write (ismdout,200) 
...
c    Restart process
c
         if(tsmd .ne. 0.0d0) write (ismdout,5070) tsmd
...
c
      if (init) then ! LOOP1
      if (use_smd_velconst .or. use_smd_forconst .and. calccom) then ! LOOP2
...
c
c    Initialization of the main SMD output file
c
      if (use_smd_velconst .and. rank.eq.0) then
         write (ismdout,2000)
 2000    format (/,'SMDinfo',3x,'Step',9x,'xCOM',6x,'yCOM',6x,
     &   'zCOM',8x,'Fx',8x,'Fy',8x,'Fz')
      end if
      if (use_smd_forconst .and. rank.eq.0) then
         write (ismdout,2001)
 2001    format (/,'SMDinfo',3x,'Step',9x,'xCOM',6x,'yCOM',6x,
     &   'zCOM')
      end if
c
c    Initialization of SMD output files for atoms
c
      if (use_smd_velconst .and. use_atfol .and. rank.eq.0) then
         do i = 1, atfol
            if (tabatfol(i) .lt. 10) then
                 write(num1,'(i1)') tabatfol(i)
                 file_name1 = "SMD"//num1//".dat"
                 open(1000+tabatfol(i),file=file_name1,action='write')
                 write (1000+tabatfol(i),*) 
     &           'Index file of atom ', tabatfol(i)
                 if (use_smd_velconst) write (1000+tabatfol(i),1006)
                 close (1000+tabatfol(i))
 1006    format(/,2x,'IATOM',2x,'TIMESTEP',2x,11x,
     &   'X',4x,7x,'Y',4x,7x,'Z',4x,6x,'FX',4x,6x,'FY',4x,6x,'FZ',4x,/)
            end if
            if (tabatfol(i) .ge. 10 .and. tabatfol(i) .lt. 100) then
                 write(num2,'(i2)') tabatfol(i)
     ...
     &           'Index file of atom ', tabatfol(i)
            ...
            if (tabatfol(i) .ge. 100 .and. tabatfol(i) .lt. 1000) then
                 write(num3,'(i3)') tabatfol(i)
     ...
     &           'Index file of atom ', tabatfol(i)
            ...
            if (tabatfol(i) .ge. 1000 .and. tabatfol(i) .lt. 10000) then
                 write(num4,'(i4)') tabatfol(i)
     ...
     &           'Index file of atom ', tabatfol(i)
            ...
            if (tabatfol(i) .ge. 10000 .and. tabatfol(i) 
     &      .lt. 100000) then
                 write(num5,'(i5)') tabatfol(i)
     ...
     &           'Index file of atom ', tabatfol(i)
            ...
            if (tabatfol(i) .ge. 100000 .and. tabatfol(i)
     &      .lt. 1000000) then
                 write(num6,'(i6)') tabatfol(i)
     ...
     &           'Index file of atom ', tabatfol(i)
            ...
            if (tabatfol(i) .ge. 1000000 .and. tabatfol(i) 
     &      .lt. 10000000) then
                 write(num7,'(i7)') tabatfol(i)
