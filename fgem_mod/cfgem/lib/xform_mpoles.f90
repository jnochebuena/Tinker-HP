...
subroutine XFORM_MPOLE_matrix(A_xy,Mpole_xy,order)
  ...
  double precision,intent(in) :: A_xy(3,3)
  double precision,intent(out) :: Mpole_xy(order,order)
...
! Mpole 1 is charge... Mpole 10 is x_3,x_3 quadrupolar coefficients etc.
  ...
  common/cfgem_MxfInd/qind1,qind2,oind1,oind2,oind3,hind1,hind2,hind3,hind4
...
subroutine XFORM_MPOLE(Mpole_xy,dimxy,Mpole_in,Mpole_out,order)
  ...
  integer,intent(in) :: order,dimxy
  double precision,intent(in) :: Mpole_xy(dimxy,dimxy)
  double precision,intent(in) :: Mpole_in(*)
  double precision,intent(out) ::Mpole_out(*)
